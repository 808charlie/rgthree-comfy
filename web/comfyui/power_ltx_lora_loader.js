...
class RgthreePowerLtxLoraLoader extends RgthreeBaseServerNode {
    constructor(title = NODE_CLASS.title) {
        super(title);
        this.serialize_widgets = true;
        this.logger = rgthree.newLogSession(`[Power Ltx Lora Stack]`);
    ...
    addNewLoraWidget(lora) {
        ...
        const widget = this.addCustomWidget(new PowerLtxLoraLoaderWidget("lora_" + this.loraWidgetsCounter));
        if (lora)
            widget.setLora(lora);
    ...
    addNonLoraWidgets() {
        ...
        moveArrayItem(this.widgets, this.addCustomWidget(new PowerLtxLoraLoaderHeaderWidget()), 1);
...
RgthreePowerLtxLoraLoader.title = NodeTypesString.POWER_LTX_LORA_LOADER;
RgthreePowerLtxLoraLoader.type = NodeTypesString.POWER_LTX_LORA_LOADER;
RgthreePowerLtxLoraLoader.comfyClass = NodeTypesString.POWER_LTX_LORA_LOADER;
RgthreePowerLtxLoraLoader[_a] = {
    type: "combo",
    values: [PROP_VALUE_SHOW_STRENGTHS_SINGLE, PROP_VALUE_SHOW_STRENGTHS_SEPARATE],
};
RgthreePowerLtxLoraLoader[_b] = {
    type: "string",
};
class PowerLtxLoraLoaderHeaderWidget extends RgthreeBaseWidget {
    constructor(name = "PowerLtxLoraLoaderHeaderWidget") {
        super(name);
        this.value = { type: "PowerLtxLoraLoaderHeaderWidget" };
        this.type = "custom";
        this.hitAreas = {
    ...
    draw(ctx, node, w, posY, height) {
        ...
        if (!lowQuality) {
            ...
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin * 2;
            ctx.fillText("Other", rposX - drawNumberWidgetPart.WIDTH_TOTAL / 2, midY);
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin;
            ctx.fillText("Vid", rposX - drawNumberWidgetPart.WIDTH_TOTAL / 2, midY);
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin;
            ctx.fillText("Aud", rposX - drawNumberWidgetPart.WIDTH_TOTAL / 2, midY);
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin;
            ctx.fillText("A2V", rposX - drawNumberWidgetPart.WIDTH_TOTAL / 2, midY);
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin;
            ctx.fillText("V2A", rposX - drawNumberWidgetPart.WIDTH_TOTAL / 2, midY);
            rposX = rposX - drawNumberWidgetPart.WIDTH_TOTAL - innerMargin;
            const headerToggleWidth = height * 1.5;
            rposX = rposX - headerToggleWidth - innerMargin * 2;
            const oldColor = ctx.fillStyle;
            ctx.fillStyle = "#c66";
            ctx.fillText("Link-aud", rposX, midY);
...
class PowerLtxLoraLoaderWidget extends RgthreeBaseWidget {
    constructor(name) {
        super(name);
        this.type = "custom";
        ...
        this.hitAreas = {
            ...
            v2aDec: { bounds: [0, 0], onClick: (e, p, n) => this.onMultDecDown(e, p, n, 'v2a') },
            v2aVal: { bounds: [0, 0], onClick: (e, p, n) => this.onMultValUp(e, p, n, 'v2a') },
            v2aInc: { bounds: [0, 0], onClick: (e, p, n) => this.onMultIncDown(e, p, n, 'v2a') },
            v2aAny: { bounds: [0, 0], onMove: (e, p, n) => this.onMultAnyMove(e, p, n, 'v2a') },
            a2vDec: { bounds: [0, 0], onClick: (e, p, n) => this.onMultDecDown(e, p, n, 'a2v') },
            a2vVal: { bounds: [0, 0], onClick: (e, p, n) => this.onMultValUp(e, p, n, 'a2v') },
            a2vInc: { bounds: [0, 0], onClick: (e, p, n) => this.onMultIncDown(e, p, n, 'a2v') },
            a2vAny: { bounds: [0, 0], onMove: (e, p, n) => this.onMultAnyMove(e, p, n, 'a2v') },
            audDec: { bounds: [0, 0], onClick: (e, p, n) => this.onMultDecDown(e, p, n, 'aud') },
            audVal: { bounds: [0, 0], onClick: (e, p, n) => this.onMultValUp(e, p, n, 'aud') },
            audInc: { bounds: [0, 0], onClick: (e, p, n) => this.onMultIncDown(e, p, n, 'aud') },
            audAny: { bounds: [0, 0], onMove: (e, p, n) => this.onMultAnyMove(e, p, n, 'aud') },
            vidDec: { bounds: [0, 0], onClick: (e, p, n) => this.onMultDecDown(e, p, n, 'vid') },
            vidVal: { bounds: [0, 0], onClick: (e, p, n) => this.onMultValUp(e, p, n, 'vid') },
            vidInc: { bounds: [0, 0], onClick: (e, p, n) => this.onMultIncDown(e, p, n, 'vid') },
            vidAny: { bounds: [0, 0], onMove: (e, p, n) => this.onMultAnyMove(e, p, n, 'vid') },
            otherDec: { bounds: [0, 0], onClick: (e, p, n) => this.onMultDecDown(e, p, n, 'other') },
            otherVal: { bounds: [0, 0], onClick: (e, p, n) => this.onMultValUp(e, p, n, 'other') },
            otherInc: { bounds: [0, 0], onClick: (e, p, n) => this.onMultIncDown(e, p, n, 'other') },
            otherAny: { bounds: [0, 0], onMove: (e, p, n) => this.onMultAnyMove(e, p, n, 'other') },
            link: { bounds: [0, 0], onDown: this.onLinkDown },
    ...
    draw(ctx, node, w, posY, height) {
        ...
        if (!this.value.on) {
            ctx.globalAlpha = app.canvas.editor_alpha * 0.4;        // 808 I recommend 0.8 
        }
        ctx.fillStyle = LiteGraph.WIDGET_TEXT_COLOR;
        let rposX = node.size[0] - margin - innerMargin - innerMargin;
        ...
        if (this.showModelAndClip) {
          ...
          {
            const multLabels = ['other', 'vid', 'aud', 'a2v', 'v2a'];
            const linkOn = this.value.link;
            for (const prop of multLabels) {
                rposX -= innerMargin;
                const multVal = this.value[prop] !== undefined ? this.value[prop] : 1;
                const linkedRed = linkOn && (prop === 'v2a' || prop === 'a2v' || prop === 'aud') ? "#c66" : undefined;
                const [la, tv, ra] = drawNumberWidgetPart(ctx, {
                    posX: rposX,
                    posY,
                    height,
                    value: multVal,
                ...
                this.hitAreas[prop + 'Dec'].bounds = la;
                this.hitAreas[prop + 'Val'].bounds = tv;
                this.hitAreas[prop + 'Inc'].bounds = ra;
                this.hitAreas[prop + 'Any'].bounds = [la[0], ra[0] + ra[1] - la[0]];
                rposX = la[0] - innerMargin;
            }
        }
        {
            const v2aDecLeft = this.hitAreas.v2aDec.bounds[0];
            const toggleWidth = height * 1.5;
            const linkPos = v2aDecLeft - toggleWidth - innerMargin * 2;
            this.hitAreas.link.bounds = drawTogglePart(ctx, { posX: linkPos, posY, height, value: this.value.link });
            rposX = this.hitAreas.link.bounds[0] - innerMargin;
        }
        const infoIconSize = height * 0.66;
        const infoWidth = infoIconSize + innerMargin + innerMargin;
        if (CONFIG_SERVICE.getConfigValue("nodes.power_ltx_lora_loader.show_info_badge")) {
            rposX -= innerMargin;
    ...
    serializeValue(node, index) {
        var _c;
        const v = { ...this.value };
        v.link = v.link !== undefined ? v.link : false;
        v.v2a = v.v2a !== undefined ? v.v2a : 1;
        v.a2v = v.a2v !== undefined ? v.a2v : 1;
        v.aud = v.aud !== undefined ? v.aud : 1;
        v.vid = v.vid !== undefined ? v.vid : 1;
        v.other = v.other !== undefined ? v.other : 1;
        if (!this.showModelAndClip) {
    ...
    onMultDecDown(event, pos, node, prop) {
        this.stepMult(-1, prop);
    }
    onMultIncDown(event, pos, node, prop) {
        this.stepMult(1, prop);
    }
    onMultAnyMove(event, pos, node, prop) {
        if (event.deltaX) {
            this.haveMouseMovedStrength = true;
            const val = ((this.value[prop] !== undefined ? this.value[prop] : 1)) + event.deltaX * 0.05;
            this.value[prop] = val;
            if (this.value.link && ['v2a', 'a2v', 'aud'].includes(prop)) {
                this.value.v2a = val;
                this.value.a2v = val;
                this.value.aud = val;
            }
    ...
    onMultValUp(event, pos, node, prop) {
        if (this.haveMouseMovedStrength)
            return;
        const canvas = app.canvas;
        const linked = this.value.link && ['v2a', 'a2v', 'aud'].includes(prop);
        canvas.prompt("Value", this.value[prop], (v) => {
            const num = Number(v);
            this.value[prop] = num;
            if (linked) {
                this.value.v2a = num;
    ...
    stepMult(direction, prop) {
        let step = 0.05;
        let val = ((this.value[prop] !== undefined ? this.value[prop] : 1)) + step * direction;
    ...
    onLinkDown(event, pos, node) {
        this.value.link = !this.value.link;
        if (this.value.link) {
            const aud = this.value.aud !== undefined ? this.value.aud : 1;
            this.value.v2a = aud;
            this.value.a2v = aud;
        }
        this.cancelMouseDown();
...
const NODE_CLASS = RgthreePowerLtxLoraLoader;
app.registerExtension({
    name: "rgthree.PowerLtxLoraLoader",
